  ...
  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] h-64 md:h-96">
          {cards.map((card, index) => {
            const image = (
              <div className="relative w-full h-full overflow-hidden rounded-[14px]">
                ...
                <div className="absolute bottom-0 inset-x-0 h-1/2 bg-gradient-to-t from-black/80 to-transparent z-20" />
                {card.alt && (
                  <span className="absolute bottom-3 inset-x-0 text-center text-white text-sm font-semibold z-30 drop-shadow-md">
                    {card.alt}
                  </span>
                )}
              </div>
            ...
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card block cursor-pointer absolute w-32 h-48 md:w-48 md:h-72">{image}</a>
            ) : (
              <div key={index} className="fan-card absolute w-32 h-48 md:w-48 md:h-72">{image}</div>
            );
          })}
        </div>
      </div>
