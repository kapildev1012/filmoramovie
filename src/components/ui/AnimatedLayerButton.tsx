// src/components/ui/AnimatedLayerButton.tsx
//
// Adapted from the shadcn-style "AnimatedLayerButton" snippet to fit this
// project's conventions:
//   • relative `cn` import  — the repo has no `@/` path alias (see tsconfig).
//   • project color tokens  — this repo's --primary / --foreground are *hex*
//     values, not HSL triplets, so `hsl(var(--primary))` would be invalid.
//     We consume the real design tokens (--color-accent-from / --color-text)
//     directly via Tailwind arbitrary values.
//   • `animate-[spin_5s_linear_infinite]` — Tailwind's built-in `spin`
//     keyframe at 5s, so no tailwind.config / @theme change is required.
  ...
  return (
    <button
      className={cn(
        ...
        "cursor-pointer bg-[var(--color-accent-from)] shadow-[8px_8px_0px_var(--color-text)] transition-all duration-300 ease-in-out",
        // "Pushed" effect on hover
        "hover:translate-y-[5px] hover:shadow-[3px_3px_0px_var(--color-text)]",
        // Focus a11y
        "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent-from)] focus-visible:ring-offset-2",
    ...
    >
      ...
      >
        ...
        <path fill="var(--color-accent-from)" d="M1006.72,744.28c2.81,143.23-110.17,257.35-247.42,261.9C613.15,1011,498.22,895.93,493.71,758.88,488.93,613.71,603,498,740.69,493.28,886.73,488.24,1004,603.87,1006.72,744.28Z" transform="translate(-202.29 -201.89)"/>
      ...
      {/* Text layer — becomes transparent on hover to reveal the spinning logo */}
      <span className="z-10 font-semibold text-white transition-colors duration-300 group-hover:text-transparent text-[1.1em]">
        {children}
      </span>
    </button>
  );
