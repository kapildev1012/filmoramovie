...
interface Props {
  /** Words to cycle through (default: LOADING → ASSEMBLING → FINALIZING) */
  words?: string[];
}

/**
 * KineticTypographyLoader — characters fly in from random 3D positions to form
 * a word, hold, then fly out; the next word takes over. Pure CSS keyframes
 * (`fly-in` / `fly-out`) defined in global.css.
 */
export const KineticTypographyLoader = ({
  words = ['LOADING', 'ASSEMBLING', 'FINALIZING'],
}: Props) => {
